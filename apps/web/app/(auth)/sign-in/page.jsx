...
export default function SignInPage() {
  ...
  const [ytmusicStatus, setYtmusicStatus] = useState('checking');
  ...
  const checkYTMusicConnection = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Redirect to library with YTMusic
        window.location.href = '/library?service=ytmusic';
      } else {
        setYtmusicStatus('not-connected');
      }
    } catch (error) {
      console.error('YTMusic connection check failed:', error);
      setYtmusicStatus('not-connected');
    }
  };

  const handleYTMusicClick = () => {
    if (ytmusicStatus === 'connected') {
      window.location.href = '/library?service=ytmusic';
    } else {
  ...
  // Check YTMusic status on component mount
  useEffect(() => {
    ...
    <div className="p-6 flex flex-col justify-center items-center h-80 w-fit bg-[#000000] rounded-lg glow-shadow--soft p-6 bg-black rounded-lg border border-white/40" id="login-box">
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span>🎬</span>
            {ytmusicStatus === 'checking' && 'Checking YTMusic...'}
            {ytmusicStatus === 'connected' && 'Continue with YouTube Music'}
            {ytmusicStatus === 'not-connected' && 'Connect YouTube Music'}
            {ytmusicStatus === 'backend-offline' && 'YTMusic (Setup Required)'}
          </button>
          <div className="h-0.5 w-20 bg-[#6A6A6A]"></div>
        </div>
      </div>
          ...
          <div className="bg-gray-900 p-6 rounded-lg max-w-md w-full mx-4 border border-white/20">
            ...
            <div className="space-y-4 text-sm text-gray-300">
              ...
              <div className="bg-gray-800 p-4 rounded border border-gray-700">
                <h4 className="font-semibold text-white mb-2">Step 1: Install Chrome Extension</h4>
                <ol className="list-decimal list-inside space-y-1">
                  <li>Open Chrome and go to <code className="bg-gray-700 px-1 rounded">chrome://extensions</code></li>
                  <li>Enable "Developer mode"</li>
                  <li>Click "Load unpacked" and select the extension folder from <code className="bg-gray-700 px-1 rounded">apps/backend/extension/</code></li>
                </ol>
              </div>
              ...
              <div className="bg-gray-800 p-4 rounded border border-gray-700">
                <h4 className="font-semibold text-white mb-2">Step 3: Start Backend</h4>
                <p>Make sure the Vybe backend is running:</p>
                <code className="bg-gray-700 px-2 py-1 rounded block mt-1">cd apps/backend && ./run.sh</code>
              </div>
            </div>
