...
export default function AddFriendsModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="vybe-aurora glass-card rounded-2xl p-6 w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-400/20 rounded-lg border border-blue-400/30">
              <UserPlus className="h-5 w-5 text-blue-400" />
            </div>
            <h2 className="page-title text-xl">Add Friends</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSearch} className="mb-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50"
        ...
        <button
          ...
          disabled={loading}
          className="w-full mb-4 px-4 py-2 bg-white/10 hover:bg-white/20 active:bg-white/20 border border-white/20 rounded-lg text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
        >
        ...
        <div className="space-y-2 overflow-y-auto flex-1 min-h-0 pr-2 modal-scroll">
          ...
          {users.length === 0 && searchQuery && !searching && (
            <div className="text-center py-12">
              <UserPlus className="h-16 w-16 text-white/60 mx-auto mb-4" />
